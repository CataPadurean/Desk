// GENERAT de update_data.py / Claude — nu edita manual.
window.ANALYSIS_DATA = {
 "generated": "2026-08-10",
 "regime": "RISK-ON, dar nu „curat”: Dow record 53.056, S&P +0,7%, Nasdaq +1,1% (rally semiconductoare), VIX 16,06. DAR randamentele urcă din nou (10Y US 4,50%, maxim de 2 săptămâni, pe revenirea petrolului) și fast money e la extrem dublu — USD long de deceniu ($39,8 miliarde, a 8-a săptămână) + SOFR short record. Săptămâna se joacă pe evenimente, nu pe trend: RBNZ 8.07 (hike devenit consens) și minute FOMC 8.07 (pivotul USD). BoC + MPR abia 15.07.",
 "regime_date": "2026-07-05",
 "sentiment": {
  "label": "RISK-ON (fragil)",
  "comment": "Apetit de risc intact — indici la record, VIX 16 — favorabil monedelor pro-ciclice „bătute” (CAD, NZD, AUD). CACIB: CAD și CHF corelate pozitiv cu risk-on, GBP negativ. Fragilitate: randamentele urcă și poziționarea USD e la extrem de deceniu; un shock de randamente sau minute FOMC hawkish poate inversa brusc tonul. Nu e un risk-on de urmărit orbește."
 },
 "currencies": {
  "USD": {
   "core": true,
   "bias": "Bearish tactic / Neutru mediu termen (risc de reversal pe poziționare)",
   "cb": "Fed hold 3,50-3,75% (17.06), ton hawkish (dot median 3,8% = un hike încă pe masă), dar NFP +57k a tăiat hike-ul din septembrie sub 50%. Minute FOMC miercuri 8.07 (21:00 RO) = pivotul: hawkish → USD relansat; non-hawkish → continuă repricing-ul dovish.",
   "banks": "Singura casă cu teză direcțională pe dolar e ING, care miza pe repricing hawkish al Fed-ului și ținte DXY 101,7-101,80 — doar că raportul a fost scris înainte de datele pieței muncii, iar premisa lui s-a rupt: angajările au ratat consensul, chiar dacă rata șomajului a rezistat. CACIB nu ia poziție pe direcție, dar semnalează ceva mai util pentru execuție: corelația negativă dintre dolar și apetitul de risc se adâncește, adică dolarul se comportă tot mai mult ca refugiu. Concluzia blocului: nicio casă nu mai apără un long de dolar cu convingere, dar nici nu-l atacă frontal."
  },
  "EUR": {
   "core": true,
   "bias": "Neutru",
   "cb": "ECB a urcat recent și ține ușa deschisă, dar cu HICP la 2,8% argumentul pentru încă un pas se subțiază de la o lună la alta. Nu are motive să grăbească nimic, iar piața a început să scoată din preț următorul hike. Decizia din 23.07 e un test de ton, nu de dobândă.",
   "banks": "UniCredit dă tonul și îl rezumă într-o formulă: perechea e la răscruce, fiindcă niciunul dintre cele două blocuri nu are un motiv propriu să tragă cursul într-o parte. Observația lor operațională e că legătura cu diferența de randamente germano-americane, ruptă o vreme, a redevenit motorul principal — deci mișcările urmăresc spread-ul, nu titlurile. CACIB și ING merg pe aceeași linie dovish privind dezinflația, iar inflația sub consens le confirmă premisa. Nicio casă nu propune o direcție: consensul e că, fără declanșator din afară, rămâne piață de intervale."
  },
  "GBP": {
   "core": true,
   "bias": "Bearish (fragil) / WATCH squeeze",
   "cb": "BoE e împărțită 7-2, iar economia încetinește vizibil: activitatea din servicii a căzut la minimul ultimilor doi ani și jumătate. Complicația e riscul fiscal, care ține randamentele sus și nu-i lasă să taie liniștit chiar dacă datele o cer. Decizia din 6.08 arbitrează.",
   "banks": "Lira are cea mai slabă acoperire din inbox-ul săptămânii: o singură casă, ING, ia poziție, iar aceea e bearish și explicit fragilă — o direcție pe care nici autorii ei n-o apără cu tărie. Argumentul de fond vine din date, nu din analiză: activitatea din servicii a căzut la minimul ultimilor doi ani și jumătate. Nimeni nu construiește însă un caz pozitiv, dar nici nu vine cu o teză bear completă, cu ținte și orizont. Blocul bancar nu oferă nici convingere, nici catalizator — lira rămâne subiect de urmărit, nu de tranzacționat."
  },
  "CAD": {
   "core": true,
   "bias": "Bullish (în construcție spre BoC 15.07)",
   "cb": "BoC stă pe loc cu inflația la 3,2% — prea sus ca să taie, prea jos ca să urce. Decizia și raportul din 15.07 sunt evenimentul Tier 1 și catalizatorul tezei pe CAD. CIBC merge mai departe: dobânda nu se mai mișcă anul acesta, adică hold prelungit peste date care se ameliorează.",
   "banks": "Singurul bloc din inbox unde trei case spun același lucru din unghiuri diferite. CACIB dă teza și nivelurile: decuplarea economiei canadiene de cea americană e exagerată, iar USDCAD ar trebui să coboare de la 1,418 spre 1,35-1,40. Scotiabank o susține cu datele de creștere, care au bătut clar așteptările după o serie de luni slabe. CIBC adaugă piesa de politică monetară: dobânda nu se mai mișcă anul acesta, deci hold prelungit peste date în ameliorare — inflația de bază la minim de cinci ani și șomajul în scădere confirmă. Rar se aliniază atâtea case pe aceeași monedă."
  },
  "JPY": {
   "core": false,
   "bias": "Bearish (dar risc mare de squeeze) — WATCH",
   "cb": "BoJ a urcat la 1,00% cu vot larg, iar majoritatea board-ului vede încă un pas anul acesta — direcția e clară, viteza nu. Inflația de bază sub țintă le permite să normalizeze în ritmul lor. Yenul are argumente pe termen lung, dar niciun catalizator imediat. Decizie 31.07.",
   "banks": "Yenul e locul unde blocul bancar se rupe în două, și aceasta e informația cea mai valoroasă a săptămânii. CACIB și-a urcat ținta față de raportul anterior și vede USDJPY la 162-163, pe argumentul că diferența de dobândă plătește poziția, iar normalizarea japoneză merge prea încet ca să conteze — inflația rămâne sub țintă, ceea ce le dă dreptate. MUFG atacă aceeași poziție din partea cealaltă: nu contestă logica de carry, ci spune că trade-ul e prea aglomerat și se golește violent. Argumente solide, concluzii opuse — o contradicție de manual, care anulează direcția netă."
  },
  "CHF": {
   "core": false,
   "bias": "Neutru-Bullish franc",
   "cb": "SNB ține dobânda la zero și sugerează că acolo rămâne până la finalul anului: cu inflația la 0,6% nu are nici motiv să urce, nici spațiu să coboare. Instrumentul real nu mai e dobânda, ci intervenția pe curs — o întărire amplă a francului vine cu risc de intervenție.",
   "banks": "Nicio casă nu deschide o teză pe franc în această săptămână, dar CACIB semnalează o anomalie care merită reținută: francul se corelează pozitiv cu apetitul de risc, adică se comportă atipic pentru o monedă de refugiu. Observația contează pentru corelații — ca acoperire într-un scenariu risk-off, francul poate să nu reacționeze conform manualului. În rest, blocul bancar îl descrie ca ferm, pe cerere constantă de safe-haven, susținut de o inflație aproape inexistentă. Fără teză și fără catalizator propriu, rămâne monedă de context, nu de poziție."
  },
  "AUD": {
   "core": false,
   "bias": "Neutru-Bullish",
   "cb": "RBA a ținut 4,35%, dar mesajul a fost mai dur decât gestul: Bullock spune că pauza nu e finalul ciclului și că mai există spațiu de urcare. Ghidajul e mai hawkish decât ce prețuiește piața, deci există loc de repricing pro-AUD. Lipsește catalizatorul propriu — următorul e 5.08.",
   "banks": "Dolarul australian nu are un raport dedicat, ci intră într-o temă mai largă: CACIB îl încadrează în grupul „monedelor bătute”, valutele pedepsite excesiv în lunile trecute și care urmează să fie reevaluate pe măsură ce narativul pesimist se golește. E o teză de coș, nu de pereche — argumentul nu ține de Australia, ci de rotația generală. Datele proprii ar susține-o parțial: inflația de bază accelerează și piața muncii rămâne solidă, ceea ce ține banca centrală sus. Fără raport dedicat, rămâne o poziție de urmărit, nu de deschis."
  },
  "NZD": {
   "core": false,
   "bias": "Bullish tactic (event RBNZ 8.07)",
   "cb": "RBNZ a ținut 2,25% într-o ședință atât de strânsă încât votul 3-3 a fost spart de Guvernator. Pentru 8.07, 22 din 28 de economiști așteaptă majorare la 2,50% — consens, deci deja prețuit. Nuanța: prăbușirea petrolului sub nivelul asumat slăbește argumentul inflaționist.",
   "banks": "Pe kiwi, sinteza vine din TMV, nu dintr-un raport de bancă comercială, și e formulată prudent: drumul cu cea mai mică rezistență duce spre o monedă mai fermă până la decizie. Argumentul din date e solid — inflația a urcat la vârf de ciclu, iar creșterea trimestrială e cea mai bună din grup. Ce merită reținut e avertismentul care însoțește teza: câștigurile sunt văzute ca limitate chiar și în cazul unei majorări, fiindcă piața a prețuit-o deja. Direcția e probabil corectă, dar spațiul de mișcare e mic — monedă secundară, doar la setup curat."
  }
 },
 "pricing": {
  "USD": {
   "view_bp": 25,
   "next_meeting": "Septembrie sub 50% după NFP +57k",
   "market": "Post-NFP, piața dă hike-ului din septembrie sub 50% și prețuiește dobânda plată până la finalul anului",
   "view": "Dot-ul median (3,8%) ține încă un hike pe masă — minutele din 8.07 arbitrează",
   "edge": "Piața prețuiește deja mai mult decât implică dot-urile; edge-ul e în ton, nu în nivel",
   "policy_rate": 3.75,
   "yield": 4.25,
   "tenor": "2Y",
   "market_bp": 50,
   "market_delta_bp": 2,
   "edge_bp": -25,
   "dir": "ANTI-USD",
   "method": "(2Y 4.25 − politică 3.75) × 100",
   "group_avg_bp": 55,
   "rel_bp": -5
  },
  "EUR": {
   "view_bp": 25,
   "next_meeting": "23.07 fără mișcare; următorul hike aproape scos din preț",
   "market": "Următorul hike ECB aproape scos din preț după HICP 2,8%",
   "view": "Un singur pas rămas plauzibil pe orizontul de doi ani, fără grabă",
   "edge": "Pricing și politică spun același lucru — fără edge exploatabil",
   "policy_rate": 2.25,
   "yield": 2.666,
   "tenor": "2Y",
   "market_bp": 42,
   "market_delta_bp": -10,
   "edge_bp": -17,
   "dir": "ALINIAT",
   "method": "(2Y 2.666 − politică 2.25) × 100",
   "group_avg_bp": 55,
   "rel_bp": -13
  },
  "GBP": {
   "view_bp": -50,
   "next_meeting": "6.08 arbitrează; tăieri prețuite în a doua parte a anului",
   "market": "Piața prețuiește tăieri în a doua parte a anului, pe date slabe",
   "view": "BoE împărțită 7-2 pe fond de încetinire — două tăieri pe orizont, dacă fiscalul permite",
   "edge": "Cea mai mare divergență din tabel: curba încă prețuiește înăsprire netă, banca merge invers",
   "policy_rate": 3.75,
   "yield": 4.186,
   "tenor": "2Y",
   "market_bp": 44,
   "market_delta_bp": -11,
   "edge_bp": -94,
   "dir": "ANTI-GBP",
   "method": "(2Y 4.186 − politică 3.75) × 100",
   "group_avg_bp": 55,
   "rel_bp": -11
  },
  "CAD": {
   "view_bp": 25,
   "next_meeting": "15.07 hold aproape sigur; nicio mișcare prețuită anul acesta",
   "market": "Piața nu prețuiește nicio mișcare BoC anul acesta",
   "view": "Hold prelungit (CIBC), apoi normalizare lentă — un singur pas pe doi ani",
   "edge": "Curba prețuiește cel mai mult din grup; contrazice pe față teza long CAD — de urmărit, nu de ignorat",
   "policy_rate": 2.25,
   "yield": 2.96,
   "tenor": "2Y",
   "market_bp": 71,
   "market_delta_bp": 11,
   "edge_bp": -46,
   "dir": "ANTI-CAD",
   "method": "(2Y 2.96 − politică 2.25) × 100",
   "group_avg_bp": 55,
   "rel_bp": 16
  },
  "JPY": {
   "view_bp": 50,
   "next_meeting": "31.07; încă un pas prețuit parțial pentru 2026",
   "market": "Încă un hike BoJ prețuit parțial pentru 2026",
   "view": "Majoritatea board-ului vede continuarea normalizării — doi pași pe orizont",
   "edge": "Piața prețuiește cam cât vede și desk-ul; riscul e poziționarea, nu dobânda",
   "policy_rate": 1.0,
   "yield": 1.611,
   "tenor": "2Y",
   "market_bp": 61,
   "market_delta_bp": 10,
   "edge_bp": -11,
   "dir": "ALINIAT",
   "method": "(2Y 1.611 − politică 1.0) × 100",
   "group_avg_bp": 55,
   "rel_bp": 6
  },
  "CHF": {
   "view_bp": 0,
   "next_meeting": "Zero mișcări prețuite până la finalul lui 2026",
   "market": "Zero mișcări prețuite până la finalul lui 2026",
   "view": "SNB a spus explicit că rămâne la zero; instrumentul e cursul, nu dobânda",
   "edge": "Citire pe 10Y (2Y indisponibil de la discontinuarea curbei zilnice) — de tratat cu rezervă",
   "policy_rate": 0.0,
   "yield": 0.438,
   "tenor": "10Y",
   "market_bp": 44,
   "market_delta_bp": -4,
   "edge_bp": -44,
   "dir": "ANTI-CHF",
   "method": "(10Y 0.438 − politică 0.0) × 100",
   "group_avg_bp": 55,
   "rel_bp": -11
  },
  "AUD": {
   "view_bp": 50,
   "next_meeting": "5.08; piața vede ciclul încheiat",
   "market": "Piața vede ciclul RBA încheiat",
   "view": "Bullock: pauza nu e finalul ciclului, mai există spațiu de urcare — doi pași pe orizont",
   "edge": "Curba prețuiește cel mai puțin din grup, iar banca sună mai dur decât atât",
   "policy_rate": 4.35,
   "yield": 4.489,
   "tenor": "2Y",
   "market_bp": 14,
   "market_delta_bp": -2,
   "edge_bp": 36,
   "dir": "PRO-AUD",
   "method": "(2Y 4.489 − politică 4.35) × 100",
   "group_avg_bp": 55,
   "rel_bp": -41
  },
  "NZD": {
   "view_bp": 25,
   "next_meeting": "8.07, majorare la 2,50% prețuită integral (22 din 28)",
   "market": "Hike-ul din 8.07 prețuit aproape integral (consens 22 din 28)",
   "view": "Majorarea e în preț; după ea, ciclul se apropie de final",
   "edge": "Necuantificabil — randamentele neozeelandeze lipsesc (RBNZ 403). Edge-ul rămâne calitativ: doar tonul mai poate mișca",
   "policy_rate": 2.5,
   "yield": 3.65,
   "tenor": "2Y",
   "market_bp": 115,
   "market_delta_bp": -2,
   "edge_bp": -90,
   "dir": "ANTI-NZD",
   "method": "(2Y 3.65 − politică 2.5) × 100",
   "group_avg_bp": 55,
   "rel_bp": 60
  }
 },
 "playbook": [
  {
   "event": "Minute FOMC",
   "date": "miercuri 8.07, 21:00",
   "scenarios": [
    {
     "name": "Hawkish",
     "odds": "~35%",
     "reaction": "USD relansat pe toată linia; randamentele urcă",
     "action": "Short USDCAD moare înainte de BoC; fără intrări anti-USD — se rămâne pe margine"
    },
    {
     "name": "Bază (echilibrat)",
     "odds": "~45%",
     "reaction": "Repricing-ul dovish continuă lent; USD moale",
     "action": "Tezele rămân ARMED — se caută confirmarea tehnică pe USDCAD"
    },
    {
     "name": "Dovish",
     "odds": "~20%",
     "reaction": "USD vândut agresiv; risk-on se amplifică",
     "action": "Cel mai bun scenariu pentru ambele teze — execuție la trigger, fără creșterea riscului"
    }
   ]
  },
  {
   "event": "Decizie RBNZ",
   "date": "miercuri 8.07, ~05:00",
   "scenarios": [
    {
     "name": "Hike + ton hawkish",
     "odds": "~40%",
     "reaction": "NZD ferm; squeeze pe short-ul record",
     "action": "Trigger-ul NZDUSD activ — intrare doar cu breakout confirmat"
    },
    {
     "name": "Hike + ton prudent",
     "odds": "~45%",
     "reaction": "Reacție inițială sus, apoi fade — hike-ul e deja în preț",
     "action": "Fără urmărire după spike; dacă intrarea nu vine în prima oră, ideea expiră"
    },
    {
     "name": "Hold",
     "odds": "~15%",
     "reaction": "NZD vândut violent; squeeze-ul se amână",
     "action": "Teza moare pe loc — se arhivează, fără repoziționare în aceeași zi"
    }
   ]
  },
  {
   "event": "Decizie BoC + MPR",
   "date": "miercuri 15.07, 16:45",
   "scenarios": [
    {
     "name": "Hold + MPR optimist",
     "odds": "~50%",
     "reaction": "CAD cumpărat pe ton; USDCAD coboară",
     "action": "Catalizatorul tezei short USDCAD — execuție la confirmarea tehnică"
    },
    {
     "name": "Hold neutru",
     "odds": "~35%",
     "reaction": "Reacție mică; direcția o dă petrolul",
     "action": "Teza rămâne validă dar fără combustibil — se ține doar cu petrol peste prag"
    },
    {
     "name": "Dovish surprise",
     "odds": "~15%",
     "reaction": "CAD vândut; invalidarea CIBC se activează",
     "action": "Teza se închide și se arhivează INVALIDATĂ — fără discuție"
    }
   ]
  }
 ],
 "reports": [
  {
   "bank": "CACIB",
   "date": "01.07",
   "asset": "USDCAD",
   "dir": "BEARISH",
   "horizon": "1-3 luni",
   "conviction": 4,
   "args": [
    "Piața a exagerat decuplarea economiei canadiene de cea americană",
    "Temă mai largă: monedele bătute se reevaluează",
    "CAD urcă odată cu apetitul de risc"
   ],
   "flow": "",
   "levels": "1,418 → ținte 1,35-1,40",
   "invalidation": "Minute FOMC hawkish / redresare bruscă a USD global",
   "change": "REVIZUIT (întărit)"
  },
  {
   "bank": "Scotiabank",
   "date": "30.06",
   "asset": "CAD",
   "dir": "BULLISH",
   "horizon": "1-2 luni",
   "conviction": 3,
   "args": [
    "Creșterea din aprilie a bătut clar așteptările",
    "Trimestrul doi se conturează peste ce prețuia piața",
    "Cererea internă se stabilizează, nu doar exporturile"
   ],
   "flow": "",
   "levels": "—",
   "invalidation": "Date canadiene slabe / petrol în cădere",
   "change": "întărit"
  },
  {
   "bank": "CIBC",
   "date": "30.06",
   "asset": "CAD",
   "dir": "BULLISH",
   "horizon": "restul anului",
   "conviction": 3,
   "args": [
    "Anul acesta e despre revenire, nu despre recesiune",
    "Nicio mișcare de dobândă până la finalul anului",
    "Hold prelungit pe date în ameliorare = context ideal"
   ],
   "flow": "",
   "levels": "—",
   "invalidation": "BoC dovish surprise la MPR 15.07",
   "change": "NOU"
  },
  {
   "bank": "ING",
   "date": "02.07",
   "asset": "USD (DXY)",
   "dir": "BULLISH",
   "horizon": "1-2 săptămâni",
   "conviction": 3,
   "args": [
    "Publicată ÎNAINTE de raportul pieței muncii — premisa s-a schimbat",
    "Mizau pe repricing hawkish al Fed",
    "Diferența de dobândă rămâne favorabilă dolarului"
   ],
   "flow": "",
   "levels": "DXY 101,7 / 101,80",
   "invalidation": "Deja parțial invalidată de NFP +57k (miss)",
   "change": "slăbit (invalidat parțial)"
  },
  {
   "bank": "UniCredit",
   "date": "01.07",
   "asset": "EURUSD",
   "dir": "NEUTRU",
   "horizon": "1 lună",
   "conviction": 2,
   "args": [
    "La răscruce: niciun bloc nu are motiv propriu să tragă cursul",
    "Spread-ul Bund-UST a redevenit motorul mișcării",
    "Fără declanșator extern, e piață de intervale"
   ],
   "flow": "",
   "levels": "—",
   "invalidation": "—",
   "change": "ÎNTORS (din direcțional în neutru)"
  },
  {
   "bank": "CACIB",
   "date": "01.07",
   "asset": "USDJPY",
   "dir": "BULLISH",
   "horizon": "1-2 luni",
   "conviction": 4,
   "args": [
    "Diferența de dobândă plătește poziția pe toată durata deținerii",
    "Normalizarea japoneză merge prea încet",
    "Și-au urcat ținta față de raportul anterior"
   ],
   "flow": "",
   "levels": "ținte 162-163",
   "invalidation": "BoJ accelerează normalizarea / risk-off global",
   "change": "REVIZUIT ↑"
  },
  {
   "bank": "MUFG",
   "date": "02.07",
   "asset": "USDJPY",
   "dir": "BEARISH",
   "horizon": "1-2 luni",
   "conviction": 3,
   "args": [
    "Trade-ul de carry e aglomerat și se golește violent",
    "Poziționarea contra yenului, la extrem de doi ani",
    "Iulie e istoric favorabil yenului"
   ],
   "flow": "",
   "levels": "—",
   "invalidation": "BoJ amână hike-ul peste octombrie",
   "change": "întărit"
  }
 ],
 "reports_meta": {
  "week": "29.06–03.07.2026",
  "processed": 7,
  "excluded": 2,
  "note": "Inbox gol la momentul procesării — sinteza acoperă batch-ul precedent. Excluse: 2 note duplicate de strategie fără teză nouă."
 },
 "indicators": {
  "USD": {
   "gdp": "+1,5%",
   "rate": "3,75%",
   "cpi": "3,5%",
   "unemp": "4,2%",
   "surprise": "mixt",
   "read": "Creștere pe jumătate, dar inflația a cedat în sfârșit și piața muncii rezistă — un mix care nu forțează Fed-ul."
  },
  "EUR": {
   "gdp": "+0,4%",
   "rate": "2,25%",
   "cpi": "2,9%",
   "unemp": "6,3%",
   "surprise": "mixt",
   "read": "Economia accelerează, dar inflația a urcat din nou pe energie — ECB rămâne blocată între cele două."
  },
  "GBP": {
   "gdp": "+0,6%",
   "rate": "3,75%",
   "cpi": "2,6%",
   "unemp": "4,9%",
   "surprise": "negativ",
   "read": "Inflația scade mai mult decât se aștepta, dar șomajul e cel mai mare din grup — cea mai slabă combinație."
  },
  "CAD": {
   "gdp": "0,0%",
   "rate": "2,25%",
   "cpi": "2,8%",
   "unemp": "6,5%",
   "surprise": "pozitiv",
   "read": "Creșterea a stagnat, însă inflația de bază e la minim de cinci ani și șomajul scade — BoC poate sta liniștită."
  },
  "JPY": {
   "gdp": "+0,5%",
   "rate": "1,00%",
   "cpi": "1,7%",
   "unemp": "2,5%",
   "surprise": "neutru",
   "read": "Inflația accelerează dar rămâne sub țintă, cu cea mai strânsă piață a muncii din grup — normalizare fără grabă."
  },
  "CHF": {
   "gdp": "+0,4%",
   "rate": "0,00%",
   "cpi": "0,5%",
   "unemp": "2,9%",
   "surprise": "neutru",
   "read": "Inflație aproape inexistentă și șomaj minim — nimic de rezolvat, deci singura variabilă rămâne cursul."
  },
  "AUD": {
   "gdp": "+0,3%",
   "rate": "4,35%",
   "cpi": "3,8%",
   "unemp": "4,4%",
   "surprise": "mixt",
   "read": "Creșterea dezamăgește, dar inflația de bază accelerează și angajările explodează — RBA are motiv să stea sus."
  },
  "NZD": {
   "gdp": "+0,8%",
   "rate": "2,50%",
   "cpi": "3,9%",
   "unemp": "5,3%",
   "surprise": "pozitiv",
   "read": "Cea mai bună creștere din grup și inflația la vârf de ciclu — de aici prima majorare din trei ani."
  }
 },
 "regime_calc": {
  "updated": "2026-08-10",
  "asof": "2026-08-07",
  "score": 2,
  "raw": 2.0,
  "label": "RISK-ON",
  "vix": 14.9,
  "components": [
   {
    "key": "vix",
    "label": "VIX",
    "value": 14.9,
    "pctile": 8,
    "score": 2,
    "weight": 0.35,
    "asof": "2026-08-07",
    "why": "VIX 14.90, percentila 8 pe 1 an"
   },
   {
    "key": "credit",
    "label": "HY OAS",
    "value": 2.7,
    "pctile": 9,
    "score": 2,
    "weight": 0.35,
    "asof": "2026-08-07",
    "why": "spread high yield 2.70%, percentila 9 pe 1 an"
   },
   {
    "key": "momentum",
    "label": "S&P momentum",
    "value": 2.41,
    "pctile": null,
    "score": 2,
    "weight": 0.3,
    "asof": "2026-08-07",
    "why": "S&P +2.41% pe 20 de ședințe, +3.58% pe 5"
   }
  ],
  "why": "VIX 14.90, percentila 8 pe 1 an · spread high yield 2.70%, percentila 9 pe 1 an · S&P +2.41% pe 20 de ședințe, +3.58% pe 5",
  "weights": {
   "vix": 0.35,
   "credit": 0.35,
   "momentum": 0.3
  },
  "status": "ok"
 },
 "trades_fx": [
  {
   "instrument": "EURUSD",
   "dir": "STAI",
   "strat": "A",
   "conf": "—",
   "rationale": "Ambele bănci centrale au pierdut hike-uri din pricing; spread 2Y încă contra EUR; poziționare neutră; criteriul 1 neutru → sub prag 5/7",
   "activation": "—",
   "invalidation": "Teză neclară = nu există trade (playbook §3.2)"
  },
  {
   "instrument": "GBPUSD",
   "dir": "WATCH (squeeze)",
   "strat": "A",
   "conf": "—",
   "rationale": "Leveraged Funds short percentila ~8 = combustibil de squeeze, dar Services PMI 48,8 și BoE split = fundamente slabe; lipsesc criteriile 1-2 obligatorii",
   "activation": "Doar catalizator pozitiv UK + risk-on susținut",
   "invalidation": "—"
  },
  {
   "instrument": "USDCAD",
   "dir": "SHORT (bias, build spre 15.07)",
   "strat": "A",
   "conf": "3/5",
   "rationale": "Cel mai aliniat bloc de rapoarte (CACIB 1,35-1,40; Scotia+CIBC rebound) + GDP beat + short CAD extremă (squeeze) + CAD pro-risk. Scor 5/7. Catalizatorul (BoC+MPR) e abia 15.07 → acumulare, nu urmărire.",
   "activation": "USD rămâne moale post-minute FOMC (non-hawkish) + confirmare tehnică sub nivelul-cheie",
   "invalidation": "Minute FOMC hawkish / 10Y US continuă sus / date CA slabe. Contra explicit: spread 2Y (+1,38) încă PRO-USD (criteriul 4) + USD long de deceniu poate întâi să-l împingă mai sus"
  },
  {
   "instrument": "NZDUSD",
   "dir": "LONG (tactic, event 8.07)",
   "strat": "A",
   "conf": "3/5",
   "rationale": "Hike la 2,50% devenit consens (22/28) + CPI ↑ spre 4,3% + short leveraged funds la RECORD + risk-on cu NZD pro-ciclic = profil clasic de squeeze. Scor confluență 5/7 (criteriile 1+2 prezente).",
   "activation": "Hike SAU ton clar hawkish pe 8.07 + risk-on intact + breakout tehnic din nivel (confirmare proprie)",
   "invalidation": "Hold dovish pe 8.07 → setup mort; SAU minute FOMC hawkish care relansează USD global. Onest: câștig limitat (hike deja prețuit), spread 2Y indisponibil = criteriul 4 necunoscut"
  }
 ],
 "trades_intraday": [
  {
   "instrument": "US30",
   "dir": "LONG (intraday)",
   "strat": "B",
   "conf": "3/5",
   "rationale": "Risk-on, Dow la record 53.056, VIX 16. DAR randamentele urcă și minute FOMC miercuri = risc → convingere temperată față de săptămâna trecută",
   "activation": "Breakout M5 valid din nivel M30 + VIX în scădere + top-8 componente verzi. NU în fereastra blackout de la minute FOMC (±15-30 de minute de la ora 21:00, miercuri)",
   "invalidation": "Minute FOMC hawkish / 10Y înapoi sus agresiv / VIX peste 20"
  },
  {
   "instrument": "GOLD",
   "dir": "LONG (tactic, prudent)",
   "strat": "B",
   "conf": "2,5/5",
   "rationale": "Bottom-fishing în metale prețioase (Saxo) + DXY sub 101. DAR randamentele reale urcă (10Y 4,50%) = vânt din față → convingere redusă",
   "activation": "DXY slab + randamente reale care nu mai urcă; breakout M5 din nivel M30 cu retest",
   "invalidation": "10Y US continuă peste 4,50% / DXY peste 101; downtrend-ul mare intact — NU e trade de poziție"
  }
 ],
 "cot": {
  "updated": "2026-08-10",
  "source": "CFTC TFF (Leveraged Funds)",
  "markets": {
   "EUR": {
    "category": "Leveraged Funds (TFF)",
    "as_of": "2026-08-04",
    "net": -52205,
    "delta_1w": 12993,
    "pct_oi": -6.5,
    "percentile_52w": 8,
    "extreme": "SHORT",
    "history_13w": [
     18003,
     16317,
     7296,
     12027,
     -17388,
     -8926,
     -15410,
     -32644,
     -45461,
     -53691,
     -56671,
     -65198,
     -52205
    ]
   },
   "GBP": {
    "category": "Leveraged Funds (TFF)",
    "as_of": "2026-08-04",
    "net": 38174,
    "delta_1w": -2923,
    "pct_oi": 15.5,
    "percentile_52w": 85,
    "extreme": null,
    "history_13w": [
     37302,
     31369,
     28829,
     27353,
     22312,
     16836,
     7567,
     16273,
     17979,
     28541,
     33236,
     41097,
     38174
    ]
   },
   "CAD": {
    "category": "Leveraged Funds (TFF)",
    "as_of": "2026-08-04",
    "net": -101748,
    "delta_1w": 747,
    "pct_oi": -27.9,
    "percentile_52w": 4,
    "extreme": "SHORT",
    "history_13w": [
     -37663,
     -38654,
     -38095,
     -44601,
     -58623,
     -65053,
     -83377,
     -88101,
     -85957,
     -92771,
     -98377,
     -102495,
     -101748
    ]
   },
   "JPY": {
    "category": "Leveraged Funds (TFF)",
    "as_of": "2026-08-04",
    "net": -60825,
    "delta_1w": 41165,
    "pct_oi": -14.5,
    "percentile_52w": 50,
    "extreme": null,
    "history_13w": [
     -62440,
     -64945,
     -70508,
     -88063,
     -99844,
     -96772,
     -97092,
     -115400,
     -90083,
     -90461,
     -96185,
     -101990,
     -60825
    ]
   },
   "CHF": {
    "category": "Leveraged Funds (TFF)",
    "as_of": "2026-08-04",
    "net": -10084,
    "delta_1w": -437,
    "pct_oi": -9.2,
    "percentile_52w": 8,
    "extreme": "SHORT",
    "history_13w": [
     -7708,
     -4845,
     -4823,
     -9949,
     -10785,
     -12366,
     -13816,
     -9666,
     -7218,
     -9500,
     -8897,
     -9647,
     -10084
    ]
   },
   "AUD": {
    "category": "Leveraged Funds (TFF)",
    "as_of": "2026-08-04",
    "net": 40637,
    "delta_1w": 13019,
    "pct_oi": 16.9,
    "percentile_52w": 58,
    "extreme": null,
    "history_13w": [
     55851,
     61178,
     60234,
     58800,
     42292,
     41538,
     39111,
     31777,
     29683,
     27222,
     24788,
     27618,
     40637
    ]
   },
   "NZD": {
    "category": "Leveraged Funds (TFF)",
    "as_of": "2026-08-04",
    "net": -30290,
    "delta_1w": -263,
    "pct_oi": -29.7,
    "percentile_52w": 4,
    "extreme": "SHORT",
    "history_13w": [
     -19435,
     -15336,
     -13670,
     -27725,
     -22254,
     -22187,
     -22822,
     -25285,
     -27201,
     -29002,
     -31086,
     -30027,
     -30290
    ]
   },
   "DXY": {
    "category": "Leveraged Funds (TFF)",
    "as_of": "2026-08-04",
    "net": 3849,
    "delta_1w": 5450,
    "pct_oi": 7.4,
    "percentile_52w": 100,
    "extreme": "LONG",
    "history_13w": [
     -4751,
     -11716,
     -12530,
     -11112,
     -13656,
     -1870,
     -5352,
     -5580,
     -4454,
     -4866,
     -1938,
     -1601,
     3849
    ]
   }
  }
 },
 "yields": {
  "updated": "2026-08-10",
  "status": {
   "US": "ok",
   "EUR": "ok",
   "GBP": "ok",
   "CAD": "ok",
   "JPY": "ok",
   "CHF": "parțial — 2Y indisponibil (SNB (rendoblid) 2Y: date vechi (2025-07-31) || SNB (rendoblid) 10Y: date vechi (2025-07-31) || SNB (rendoblid): fără tenoruri noi/proaspete || stooq_chf: RuntimeError: Stooq: 2ychy.b: răspuns non-CSV ('<!DOCTYPE html><html><head><meta charset=\"utf-8\"><meta name='); 10ychy.b: răspuns non-CSV ('<!DOCTYPE || SNB RSS (doar 10Y): fără tenoruri noi/proaspete || manual: RuntimeError: fără serie manuală pentru CHF)",
   "AUD": "ok",
   "NZD": "ok"
  },
  "sources": {
   "US": "2Y: FRED · 10Y: FRED",
   "EUR": "2Y: ECB (curba AAA euro area) · 10Y: ECB (curba AAA euro area)",
   "GBP": "2Y: BoE (curba GLC nominală) · 10Y: BoE (curba GLC nominală)",
   "CAD": "2Y: BoC Valet · 10Y: BoC Valet",
   "JPY": "2Y: MOF Japonia · 10Y: MOF Japonia",
   "CHF": "10Y: SNB xlsx (10Y)",
   "AUD": "2Y: RBA F2 zilnic · 10Y: RBA F2 zilnic",
   "NZD": "2Y: manual — RBNZ pagina B2 — Secondary market government bond closing yields (NZFMA, lag 1 zi) · 10Y: manual — RBNZ pagina B2 — Secondary market government bond closing yields (NZFMA, lag 1 zi)"
  },
  "levels": {
   "US": {
    "2Y": {
     "date": "2026-08-06",
     "value": 4.25,
     "delta_1w": 0.02
    },
    "10Y": {
     "date": "2026-08-06",
     "value": 4.69,
     "delta_1w": 0.01
    }
   },
   "EUR": {
    "2Y": {
     "date": "2026-08-07",
     "value": 2.666,
     "delta_1w": -0.095
    },
    "10Y": {
     "date": "2026-08-07",
     "value": 3.152,
     "delta_1w": -0.075
    }
   },
   "GBP": {
    "2Y": {
     "date": "2026-08-07",
     "value": 4.186,
     "delta_1w": -0.113
    },
    "10Y": {
     "date": "2026-08-07",
     "value": 4.994,
     "delta_1w": -0.11
    }
   },
   "CAD": {
    "2Y": {
     "date": "2026-08-07",
     "value": 2.96,
     "delta_1w": 0.11
    },
    "10Y": {
     "date": "2026-08-07",
     "value": 3.64,
     "delta_1w": 0.05
    }
   },
   "JPY": {
    "2Y": {
     "date": "2026-08-07",
     "value": 1.611,
     "delta_1w": 0.104
    },
    "10Y": {
     "date": "2026-08-07",
     "value": 2.804,
     "delta_1w": 0.003
    }
   },
   "CHF": {
    "10Y": {
     "date": "2026-08-10",
     "value": 0.438,
     "delta_1w": -0.038
    }
   },
   "AUD": {
    "2Y": {
     "date": "2026-08-05",
     "value": 4.489,
     "delta_1w": -0.017
    },
    "10Y": {
     "date": "2026-08-05",
     "value": 4.924,
     "delta_1w": -0.009
    }
   },
   "NZD": {
    "2Y": {
     "date": "2026-07-30",
     "value": 3.65,
     "delta_1w": -0.02
    },
    "10Y": {
     "date": "2026-07-30",
     "value": 4.73,
     "delta_1w": 0.01
    }
   }
  },
  "spreads": {
   "EURUSD": {
    "2Y": {
     "value": -1.584,
     "delta_1w": -0.115
    },
    "10Y": {
     "value": -1.538,
     "delta_1w": -0.085
    }
   },
   "GBPUSD": {
    "2Y": {
     "value": -0.064,
     "delta_1w": -0.133
    },
    "10Y": {
     "value": 0.304,
     "delta_1w": -0.12
    }
   },
   "AUDUSD": {
    "2Y": {
     "value": 0.239,
     "delta_1w": -0.037
    },
    "10Y": {
     "value": 0.234,
     "delta_1w": -0.019
    }
   },
   "NZDUSD": {
    "2Y": {
     "value": -0.6,
     "delta_1w": -0.04
    },
    "10Y": {
     "value": 0.04,
     "delta_1w": 0.0
    }
   },
   "USDJPY": {
    "2Y": {
     "value": 2.639,
     "delta_1w": -0.084
    },
    "10Y": {
     "value": 1.886,
     "delta_1w": 0.007
    }
   },
   "USDCHF": {
    "10Y": {
     "value": 4.252,
     "delta_1w": 0.048
    }
   },
   "USDCAD": {
    "2Y": {
     "value": 1.29,
     "delta_1w": -0.09
    },
    "10Y": {
     "value": 1.05,
     "delta_1w": -0.04
    }
   }
  },
  "history": {
   "US": {
    "2Y": [
     [
      "2026-06-25",
      4.09
     ],
     [
      "2026-06-26",
      4.07
     ],
     [
      "2026-06-29",
      4.1
     ],
     [
      "2026-06-30",
      4.14
     ],
     [
      "2026-07-01",
      4.17
     ],
     [
      "2026-07-02",
      4.14
     ],
     [
      "2026-07-06",
      4.13
     ],
     [
      "2026-07-07",
      4.19
     ],
     [
      "2026-07-08",
      4.21
     ],
     [
      "2026-07-09",
      4.16
     ],
     [
      "2026-07-10",
      4.21
     ],
     [
      "2026-07-13",
      4.26
     ],
     [
      "2026-07-14",
      4.18
     ],
     [
      "2026-07-15",
      4.13
     ],
     [
      "2026-07-16",
      4.16
     ],
     [
      "2026-07-17",
      4.18
     ],
     [
      "2026-07-20",
      4.21
     ],
     [
      "2026-07-21",
      4.26
     ],
     [
      "2026-07-22",
      4.31
     ],
     [
      "2026-07-23",
      4.37
     ],
     [
      "2026-07-24",
      4.33
     ],
     [
      "2026-07-27",
      4.31
     ],
     [
      "2026-07-28",
      4.26
     ],
     [
      "2026-07-29",
      4.22
     ],
     [
      "2026-07-30",
      4.23
     ],
     [
      "2026-07-31",
      4.28
     ],
     [
      "2026-08-03",
      4.25
     ],
     [
      "2026-08-04",
      4.2
     ],
     [
      "2026-08-05",
      4.18
     ],
     [
      "2026-08-06",
      4.25
     ]
    ],
    "10Y": [
     [
      "2026-06-25",
      4.4
     ],
     [
      "2026-06-26",
      4.38
     ],
     [
      "2026-06-29",
      4.38
     ],
     [
      "2026-06-30",
      4.44
     ],
     [
      "2026-07-01",
      4.48
     ],
     [
      "2026-07-02",
      4.49
     ],
     [
      "2026-07-06",
      4.48
     ],
     [
      "2026-07-07",
      4.55
     ],
     [
      "2026-07-08",
      4.56
     ],
     [
      "2026-07-09",
      4.54
     ],
     [
      "2026-07-10",
      4.56
     ],
     [
      "2026-07-13",
      4.62
     ],
     [
      "2026-07-14",
      4.58
     ],
     [
      "2026-07-15",
      4.55
     ],
     [
      "2026-07-16",
      4.57
     ],
     [
      "2026-07-17",
      4.55
     ],
     [
      "2026-07-20",
      4.6
     ],
     [
      "2026-07-21",
      4.63
     ],
     [
      "2026-07-22",
      4.67
     ],
     [
      "2026-07-23",
      4.71
     ],
     [
      "2026-07-24",
      4.69
     ],
     [
      "2026-07-27",
      4.65
     ],
     [
      "2026-07-28",
      4.61
     ],
     [
      "2026-07-29",
      4.67
     ],
     [
      "2026-07-30",
      4.68
     ],
     [
      "2026-07-31",
      4.75
     ],
     [
      "2026-08-03",
      4.7
     ],
     [
      "2026-08-04",
      4.63
     ],
     [
      "2026-08-05",
      4.63
     ],
     [
      "2026-08-06",
      4.69
     ]
    ]
   },
   "EUR": {
    "2Y": [
     [
      "2026-06-29",
      2.476732268
     ],
     [
      "2026-06-30",
      2.4760833371
     ],
     [
      "2026-07-01",
      2.4738910102
     ],
     [
      "2026-07-02",
      2.4543322029
     ],
     [
      "2026-07-03",
      2.4707016791
     ],
     [
      "2026-07-06",
      2.484644142
     ],
     [
      "2026-07-07",
      2.5291403048
     ],
     [
      "2026-07-08",
      2.6192816299
     ],
     [
      "2026-07-09",
      2.6146246593
     ],
     [
      "2026-07-10",
      2.5992991792
     ],
     [
      "2026-07-13",
      2.6446073262
     ],
     [
      "2026-07-14",
      2.6874859602
     ],
     [
      "2026-07-15",
      2.6761872416
     ],
     [
      "2026-07-16",
      2.6989120043
     ],
     [
      "2026-07-17",
      2.6977042564
     ],
     [
      "2026-07-20",
      2.7099382231
     ],
     [
      "2026-07-21",
      2.7373771255
     ],
     [
      "2026-07-22",
      2.7617110552
     ],
     [
      "2026-07-23",
      2.8005546841
     ],
     [
      "2026-07-24",
      2.7719528205
     ],
     [
      "2026-07-27",
      2.7295925773
     ],
     [
      "2026-07-28",
      2.7075644189
     ],
     [
      "2026-07-29",
      2.7319809842
     ],
     [
      "2026-07-30",
      2.7130737913
     ],
     [
      "2026-07-31",
      2.7607858384
     ],
     [
      "2026-08-03",
      2.7020530741
     ],
     [
      "2026-08-04",
      2.662325091
     ],
     [
      "2026-08-05",
      2.6703142615
     ],
     [
      "2026-08-06",
      2.6725142222
     ],
     [
      "2026-08-07",
      2.6658514849
     ]
    ],
    "10Y": [
     [
      "2026-06-29",
      2.922896152
     ],
     [
      "2026-06-30",
      2.9244927445
     ],
     [
      "2026-07-01",
      2.9633626284
     ],
     [
      "2026-07-02",
      2.9878649255
     ],
     [
      "2026-07-03",
      3.0061092915
     ],
     [
      "2026-07-06",
      3.0156822508
     ],
     [
      "2026-07-07",
      3.0568869592
     ],
     [
      "2026-07-08",
      3.1370395898
     ],
     [
      "2026-07-09",
      3.1292327833
     ],
     [
      "2026-07-10",
      3.1085540184
     ],
     [
      "2026-07-13",
      3.1247022419
     ],
     [
      "2026-07-14",
      3.1462568028
     ],
     [
      "2026-07-15",
      3.1544621071
     ],
     [
      "2026-07-16",
      3.1773426637
     ],
     [
      "2026-07-17",
      3.1521219932
     ],
     [
      "2026-07-20",
      3.1678464688
     ],
     [
      "2026-07-21",
      3.2100962672
     ],
     [
      "2026-07-22",
      3.2040119707
     ],
     [
      "2026-07-23",
      3.2247334899
     ],
     [
      "2026-07-24",
      3.2069910464
     ],
     [
      "2026-07-27",
      3.1657910287
     ],
     [
      "2026-07-28",
      3.1579633503
     ],
     [
      "2026-07-29",
      3.1750136066
     ],
     [
      "2026-07-30",
      3.2006565473
     ],
     [
      "2026-07-31",
      3.2273140986
     ],
     [
      "2026-08-03",
      3.1854986169
     ],
     [
      "2026-08-04",
      3.1462738472
     ],
     [
      "2026-08-05",
      3.1466241785
     ],
     [
      "2026-08-06",
      3.146678757
     ],
     [
      "2026-08-07",
      3.1523384986
     ]
    ]
   },
   "GBP": {
    "2Y": [
     [
      "2026-07-01",
      4.023306434611807
     ],
     [
      "2026-07-02",
      4.002580640272512
     ],
     [
      "2026-07-03",
      4.016434639704916
     ],
     [
      "2026-07-06",
      4.013775037124268
     ],
     [
      "2026-07-07",
      4.050602211399644
     ],
     [
      "2026-07-08",
      4.207763301217807
     ],
     [
      "2026-07-09",
      4.13725526795426
     ],
     [
      "2026-07-10",
      4.120412262796372
     ],
     [
      "2026-07-13",
      4.23097189203075
     ],
     [
      "2026-07-14",
      4.211552022110643
     ],
     [
      "2026-07-15",
      4.213406414526219
     ],
     [
      "2026-07-16",
      4.240251682440775
     ],
     [
      "2026-07-17",
      4.238958142991435
     ],
     [
      "2026-07-20",
      4.296143907807179
     ],
     [
      "2026-07-21",
      4.291527554847961
     ],
     [
      "2026-07-22",
      4.3293533493665795
     ],
     [
      "2026-07-23",
      4.389995088523126
     ],
     [
      "2026-07-24",
      4.321041273541144
     ],
     [
      "2026-07-27",
      4.271428807353065
     ],
     [
      "2026-07-28",
      4.23854097302667
     ],
     [
      "2026-07-29",
      4.325468244874113
     ],
     [
      "2026-07-30",
      4.261328145286431
     ],
     [
      "2026-07-31",
      4.299102499557623
     ],
     [
      "2026-08-03",
      4.212115975767686
     ],
     [
      "2026-08-04",
      4.162427356079027
     ],
     [
      "2026-08-05",
      4.139489886951438
     ],
     [
      "2026-08-06",
      4.180836569306957
     ],
     [
      "2026-08-07",
      4.185773794811261
     ]
    ],
    "10Y": [
     [
      "2026-07-01",
      4.830054802515342
     ],
     [
      "2026-07-02",
      4.837033481460478
     ],
     [
      "2026-07-03",
      4.853533312222898
     ],
     [
      "2026-07-06",
      4.858159044038841
     ],
     [
      "2026-07-07",
      4.897287351199506
     ],
     [
      "2026-07-08",
      5.035648073597881
     ],
     [
      "2026-07-09",
      4.975078706926774
     ],
     [
      "2026-07-10",
      4.951162478078606
     ],
     [
      "2026-07-13",
      5.028196033758737
     ],
     [
      "2026-07-14",
      5.014106531486539
     ],
     [
      "2026-07-15",
      5.0138156714730675
     ],
     [
      "2026-07-16",
      5.037610298884109
     ],
     [
      "2026-07-17",
      5.013893132702349
     ],
     [
      "2026-07-20",
      5.093882625602918
     ],
     [
      "2026-07-21",
      5.092963165462118
     ],
     [
      "2026-07-22",
      5.103199386708677
     ],
     [
      "2026-07-23",
      5.168286623894329
     ],
     [
      "2026-07-24",
      5.099101000334615
     ],
     [
      "2026-07-27",
      5.048882799046961
     ],
     [
      "2026-07-28",
      5.018604614345725
     ],
     [
      "2026-07-29",
      5.077929424519178
     ],
     [
      "2026-07-30",
      5.077978561621027
     ],
     [
      "2026-07-31",
      5.1039949579875055
     ],
     [
      "2026-08-03",
      5.021030928121406
     ],
     [
      "2026-08-04",
      4.987182325837251
     ],
     [
      "2026-08-05",
      4.956253824036943
     ],
     [
      "2026-08-06",
      4.993869273656854
     ],
     [
      "2026-08-07",
      4.994469121105159
     ]
    ]
   },
   "CAD": {
    "2Y": [
     [
      "2026-06-25",
      2.75
     ],
     [
      "2026-06-26",
      2.75
     ],
     [
      "2026-06-29",
      2.73
     ],
     [
      "2026-06-30",
      2.74
     ],
     [
      "2026-07-02",
      2.76
     ],
     [
      "2026-07-03",
      2.75
     ],
     [
      "2026-07-06",
      2.72
     ],
     [
      "2026-07-07",
      2.8
     ],
     [
      "2026-07-08",
      2.85
     ],
     [
      "2026-07-09",
      2.8
     ],
     [
      "2026-07-10",
      2.82
     ],
     [
      "2026-07-13",
      2.88
     ],
     [
      "2026-07-14",
      2.86
     ],
     [
      "2026-07-15",
      2.82
     ],
     [
      "2026-07-16",
      2.83
     ],
     [
      "2026-07-17",
      2.86
     ],
     [
      "2026-07-20",
      2.84
     ],
     [
      "2026-07-21",
      2.82
     ],
     [
      "2026-07-22",
      2.89
     ],
     [
      "2026-07-23",
      2.96
     ],
     [
      "2026-07-24",
      2.9
     ],
     [
      "2026-07-27",
      2.87
     ],
     [
      "2026-07-28",
      2.84
     ],
     [
      "2026-07-29",
      2.85
     ],
     [
      "2026-07-30",
      2.85
     ],
     [
      "2026-07-31",
      2.91
     ],
     [
      "2026-08-04",
      2.83
     ],
     [
      "2026-08-05",
      2.82
     ],
     [
      "2026-08-06",
      2.92
     ],
     [
      "2026-08-07",
      2.96
     ]
    ],
    "10Y": [
     [
      "2026-06-25",
      3.39
     ],
     [
      "2026-06-26",
      3.39
     ],
     [
      "2026-06-29",
      3.37
     ],
     [
      "2026-06-30",
      3.38
     ],
     [
      "2026-07-02",
      3.45
     ],
     [
      "2026-07-03",
      3.45
     ],
     [
      "2026-07-06",
      3.42
     ],
     [
      "2026-07-07",
      3.49
     ],
     [
      "2026-07-08",
      3.56
     ],
     [
      "2026-07-09",
      3.52
     ],
     [
      "2026-07-10",
      3.51
     ],
     [
      "2026-07-13",
      3.56
     ],
     [
      "2026-07-14",
      3.57
     ],
     [
      "2026-07-15",
      3.53
     ],
     [
      "2026-07-16",
      3.53
     ],
     [
      "2026-07-17",
      3.56
     ],
     [
      "2026-07-20",
      3.57
     ],
     [
      "2026-07-21",
      3.56
     ],
     [
      "2026-07-22",
      3.6
     ],
     [
      "2026-07-23",
      3.65
     ],
     [
      "2026-07-24",
      3.6
     ],
     [
      "2026-07-27",
      3.56
     ],
     [
      "2026-07-28",
      3.53
     ],
     [
      "2026-07-29",
      3.58
     ],
     [
      "2026-07-30",
      3.59
     ],
     [
      "2026-07-31",
      3.65
     ],
     [
      "2026-08-04",
      3.55
     ],
     [
      "2026-08-05",
      3.55
     ],
     [
      "2026-08-06",
      3.61
     ],
     [
      "2026-08-07",
      3.64
     ]
    ]
   },
   "JPY": {
    "2Y": [
     [
      "2026-07-01",
      1.4
     ],
     [
      "2026-07-02",
      1.395
     ],
     [
      "2026-07-03",
      1.39
     ],
     [
      "2026-07-06",
      1.396
     ],
     [
      "2026-07-07",
      1.402
     ],
     [
      "2026-07-08",
      1.433
     ],
     [
      "2026-07-09",
      1.444
     ],
     [
      "2026-07-10",
      1.434
     ],
     [
      "2026-07-13",
      1.451
     ],
     [
      "2026-07-14",
      1.435
     ],
     [
      "2026-07-15",
      1.436
     ],
     [
      "2026-07-16",
      1.44
     ],
     [
      "2026-07-17",
      1.436
     ],
     [
      "2026-07-21",
      1.441
     ],
     [
      "2026-07-22",
      1.45
     ],
     [
      "2026-07-23",
      1.499
     ],
     [
      "2026-07-24",
      1.531
     ],
     [
      "2026-07-27",
      1.513
     ],
     [
      "2026-07-28",
      1.504
     ],
     [
      "2026-07-29",
      1.488
     ],
     [
      "2026-07-30",
      1.497
     ],
     [
      "2026-07-31",
      1.507
     ],
     [
      "2026-08-03",
      1.562
     ],
     [
      "2026-08-04",
      1.568
     ],
     [
      "2026-08-05",
      1.569
     ],
     [
      "2026-08-06",
      1.565
     ],
     [
      "2026-08-07",
      1.611
     ]
    ],
    "10Y": [
     [
      "2026-07-01",
      2.711
     ],
     [
      "2026-07-02",
      2.778
     ],
     [
      "2026-07-03",
      2.768
     ],
     [
      "2026-07-06",
      2.823
     ],
     [
      "2026-07-07",
      2.834
     ],
     [
      "2026-07-08",
      2.856
     ],
     [
      "2026-07-09",
      2.866
     ],
     [
      "2026-07-10",
      2.761
     ],
     [
      "2026-07-13",
      2.786
     ],
     [
      "2026-07-14",
      2.713
     ],
     [
      "2026-07-15",
      2.697
     ],
     [
      "2026-07-16",
      2.719
     ],
     [
      "2026-07-17",
      2.715
     ],
     [
      "2026-07-21",
      2.731
     ],
     [
      "2026-07-22",
      2.745
     ],
     [
      "2026-07-23",
      2.776
     ],
     [
      "2026-07-24",
      2.815
     ],
     [
      "2026-07-27",
      2.778
     ],
     [
      "2026-07-28",
      2.783
     ],
     [
      "2026-07-29",
      2.757
     ],
     [
      "2026-07-30",
      2.801
     ],
     [
      "2026-07-31",
      2.801
     ],
     [
      "2026-08-03",
      2.824
     ],
     [
      "2026-08-04",
      2.848
     ],
     [
      "2026-08-05",
      2.813
     ],
     [
      "2026-08-06",
      2.773
     ],
     [
      "2026-08-07",
      2.804
     ]
    ]
   },
   "CHF": {
    "2Y": [],
    "10Y": [
     [
      "2026-06-30",
      0.31
     ],
     [
      "2026-07-01",
      0.337
     ],
     [
      "2026-07-02",
      0.356
     ],
     [
      "2026-07-03",
      0.365
     ],
     [
      "2026-07-06",
      0.364
     ],
     [
      "2026-07-07",
      0.384
     ],
     [
      "2026-07-08",
      0.41
     ],
     [
      "2026-07-09",
      0.426
     ],
     [
      "2026-07-10",
      0.44
     ],
     [
      "2026-07-13",
      0.467
     ],
     [
      "2026-07-14",
      0.485
     ],
     [
      "2026-07-15",
      0.473
     ],
     [
      "2026-07-16",
      0.483
     ],
     [
      "2026-07-17",
      0.48
     ],
     [
      "2026-07-20",
      0.49
     ],
     [
      "2026-07-21",
      0.513
     ],
     [
      "2026-07-22",
      0.499
     ],
     [
      "2026-07-23",
      0.535
     ],
     [
      "2026-07-24",
      0.537
     ],
     [
      "2026-07-27",
      0.492
     ],
     [
      "2026-07-28",
      0.438
     ],
     [
      "2026-07-29",
      0.461
     ],
     [
      "2026-07-30",
      0.482
     ],
     [
      "2026-07-31",
      0.475
     ],
     [
      "2026-08-03",
      0.476
     ],
     [
      "2026-08-04",
      0.485
     ],
     [
      "2026-08-05",
      0.428
     ],
     [
      "2026-08-06",
      0.421
     ],
     [
      "2026-08-07",
      0.441
     ],
     [
      "2026-08-10",
      0.438
     ]
    ]
   },
   "AUD": {
    "2Y": [
     [
      "2026-06-25",
      4.413
     ],
     [
      "2026-06-26",
      4.406
     ],
     [
      "2026-06-29",
      4.43
     ],
     [
      "2026-06-30",
      4.413
     ],
     [
      "2026-07-01",
      4.458
     ],
     [
      "2026-07-02",
      4.47
     ],
     [
      "2026-07-03",
      4.444
     ],
     [
      "2026-07-06",
      4.428
     ],
     [
      "2026-07-07",
      4.442
     ],
     [
      "2026-07-08",
      4.487
     ],
     [
      "2026-07-09",
      4.483
     ],
     [
      "2026-07-10",
      4.443
     ],
     [
      "2026-07-13",
      4.468
     ],
     [
      "2026-07-14",
      4.514
     ],
     [
      "2026-07-15",
      4.501
     ],
     [
      "2026-07-16",
      4.487
     ],
     [
      "2026-07-17",
      4.5
     ],
     [
      "2026-07-20",
      4.566
     ],
     [
      "2026-07-21",
      4.539
     ],
     [
      "2026-07-22",
      4.561
     ],
     [
      "2026-07-23",
      4.607
     ],
     [
      "2026-07-24",
      4.715
     ],
     [
      "2026-07-27",
      4.625
     ],
     [
      "2026-07-28",
      4.574
     ],
     [
      "2026-07-29",
      4.506
     ],
     [
      "2026-07-30",
      4.559
     ],
     [
      "2026-07-31",
      4.509
     ],
     [
      "2026-08-03",
      4.517
     ],
     [
      "2026-08-04",
      4.542
     ],
     [
      "2026-08-05",
      4.489
     ]
    ],
    "10Y": [
     [
      "2026-06-25",
      4.739
     ],
     [
      "2026-06-26",
      4.729
     ],
     [
      "2026-06-29",
      4.759
     ],
     [
      "2026-06-30",
      4.734
     ],
     [
      "2026-07-01",
      4.798
     ],
     [
      "2026-07-02",
      4.832
     ],
     [
      "2026-07-03",
      4.808
     ],
     [
      "2026-07-06",
      4.798
     ],
     [
      "2026-07-07",
      4.833
     ],
     [
      "2026-07-08",
      4.893
     ],
     [
      "2026-07-09",
      4.887
     ],
     [
      "2026-07-10",
      4.848
     ],
     [
      "2026-07-13",
      4.873
     ],
     [
      "2026-07-14",
      4.923
     ],
     [
      "2026-07-15",
      4.913
     ],
     [
      "2026-07-16",
      4.909
     ],
     [
      "2026-07-17",
      4.914
     ],
     [
      "2026-07-20",
      4.974
     ],
     [
      "2026-07-21",
      4.955
     ],
     [
      "2026-07-22",
      4.989
     ],
     [
      "2026-07-23",
      5.004
     ],
     [
      "2026-07-24",
      5.1
     ],
     [
      "2026-07-27",
      5.012
     ],
     [
      "2026-07-28",
      4.977
     ],
     [
      "2026-07-29",
      4.933
     ],
     [
      "2026-07-30",
      5.011
     ],
     [
      "2026-07-31",
      4.943
     ],
     [
      "2026-08-03",
      4.964
     ],
     [
      "2026-08-04",
      4.988
     ],
     [
      "2026-08-05",
      4.924
     ]
    ]
   },
   "NZD": {
    "2Y": [
     [
      "2026-07-17",
      3.58
     ],
     [
      "2026-07-20",
      3.65
     ],
     [
      "2026-07-21",
      3.62
     ],
     [
      "2026-07-22",
      3.63
     ],
     [
      "2026-07-23",
      3.67
     ],
     [
      "2026-07-24",
      3.77
     ],
     [
      "2026-07-27",
      3.67
     ],
     [
      "2026-07-28",
      3.68
     ],
     [
      "2026-07-29",
      3.6
     ],
     [
      "2026-07-30",
      3.65
     ]
    ],
    "10Y": [
     [
      "2026-07-17",
      4.65
     ],
     [
      "2026-07-20",
      4.71
     ],
     [
      "2026-07-21",
      4.68
     ],
     [
      "2026-07-22",
      4.7
     ],
     [
      "2026-07-23",
      4.72
     ],
     [
      "2026-07-24",
      4.79
     ],
     [
      "2026-07-27",
      4.71
     ],
     [
      "2026-07-28",
      4.72
     ],
     [
      "2026-07-29",
      4.65
     ],
     [
      "2026-07-30",
      4.73
     ]
    ]
   }
  }
 },
 "seasonality": {
  "updated": "2026-07-20",
  "years": 10,
  "instruments": {
   "EURUSD": {
    "1": {
     "avg": 0.34,
     "hit": 45,
     "n": 11
    },
    "2": {
     "avg": -0.73,
     "hit": 27,
     "n": 11
    },
    "3": {
     "avg": 0.44,
     "hit": 55,
     "n": 11
    },
    "4": {
     "avg": 0.39,
     "hit": 55,
     "n": 11
    },
    "5": {
     "avg": -0.09,
     "hit": 45,
     "n": 11
    },
    "6": {
     "avg": 0.12,
     "hit": 55,
     "n": 11
    },
    "7": {
     "avg": 0.46,
     "hit": 73,
     "n": 11
    },
    "8": {
     "avg": 0.02,
     "hit": 40,
     "n": 10
    },
    "9": {
     "avg": -0.82,
     "hit": 40,
     "n": 10
    },
    "10": {
     "avg": -0.83,
     "hit": 20,
     "n": 10
    },
    "11": {
     "avg": 0.3,
     "hit": 50,
     "n": 10
    },
    "12": {
     "avg": 1.09,
     "hit": 80,
     "n": 10
    }
   },
   "GBPUSD": {
    "1": {
     "avg": 0.81,
     "hit": 55,
     "n": 11
    },
    "2": {
     "avg": -0.91,
     "hit": 27,
     "n": 11
    },
    "3": {
     "avg": 0.1,
     "hit": 55,
     "n": 11
    },
    "4": {
     "avg": 0.65,
     "hit": 64,
     "n": 11
    },
    "5": {
     "avg": -0.56,
     "hit": 36,
     "n": 11
    },
    "6": {
     "avg": -1.1,
     "hit": 45,
     "n": 11
    },
    "7": {
     "avg": 0.44,
     "hit": 73,
     "n": 11
    },
    "8": {
     "avg": -0.58,
     "hit": 30,
     "n": 10
    },
    "9": {
     "avg": -0.69,
     "hit": 40,
     "n": 10
    },
    "10": {
     "avg": -0.58,
     "hit": 40,
     "n": 10
    },
    "11": {
     "avg": 1.14,
     "hit": 60,
     "n": 10
    },
    "12": {
     "avg": 0.77,
     "hit": 70,
     "n": 10
    }
   },
   "USDCAD": {
    "1": {
     "avg": -0.55,
     "hit": 45,
     "n": 11
    },
    "2": {
     "avg": 0.6,
     "hit": 64,
     "n": 11
    },
    "3": {
     "avg": 0.24,
     "hit": 45,
     "n": 11
    },
    "4": {
     "avg": -0.6,
     "hit": 45,
     "n": 11
    },
    "5": {
     "avg": 0.16,
     "hit": 45,
     "n": 11
    },
    "6": {
     "avg": -0.32,
     "hit": 45,
     "n": 11
    },
    "7": {
     "avg": -0.4,
     "hit": 45,
     "n": 11
    },
    "8": {
     "avg": 0.31,
     "hit": 70,
     "n": 10
    },
    "9": {
     "avg": 0.76,
     "hit": 50,
     "n": 10
    },
    "10": {
     "avg": 0.95,
     "hit": 70,
     "n": 10
    },
    "11": {
     "avg": 0.05,
     "hit": 50,
     "n": 10
    },
    "12": {
     "avg": -0.66,
     "hit": 30,
     "n": 10
    }
   },
   "USDJPY": {
    "1": {
     "avg": -0.54,
     "hit": 36,
     "n": 11
    },
    "2": {
     "avg": -0.05,
     "hit": 45,
     "n": 11
    },
    "3": {
     "avg": 0.59,
     "hit": 36,
     "n": 11
    },
    "4": {
     "avg": 0.38,
     "hit": 55,
     "n": 11
    },
    "5": {
     "avg": 0.5,
     "hit": 55,
     "n": 11
    },
    "6": {
     "avg": 0.9,
     "hit": 64,
     "n": 11
    },
    "7": {
     "avg": -0.89,
     "hit": 27,
     "n": 11
    },
    "8": {
     "avg": -0.06,
     "hit": 50,
     "n": 10
    },
    "9": {
     "avg": 1.11,
     "hit": 70,
     "n": 10
    },
    "10": {
     "avg": 1.99,
     "hit": 70,
     "n": 10
    },
    "11": {
     "avg": 0.01,
     "hit": 40,
     "n": 10
    },
    "12": {
     "avg": -0.61,
     "hit": 50,
     "n": 10
    }
   },
   "USDCHF": {
    "1": {
     "avg": -0.25,
     "hit": 55,
     "n": 11
    },
    "2": {
     "avg": 0.47,
     "hit": 64,
     "n": 11
    },
    "3": {
     "avg": 0.21,
     "hit": 45,
     "n": 11
    },
    "4": {
     "avg": -0.11,
     "hit": 55,
     "n": 11
    },
    "5": {
     "avg": -0.38,
     "hit": 27,
     "n": 11
    },
    "6": {
     "avg": -0.53,
     "hit": 27,
     "n": 11
    },
    "7": {
     "avg": -0.76,
     "hit": 27,
     "n": 11
    },
    "8": {
     "avg": -0.18,
     "hit": 40,
     "n": 10
    },
    "9": {
     "avg": 0.82,
     "hit": 70,
     "n": 10
    },
    "10": {
     "avg": 0.91,
     "hit": 60,
     "n": 10
    },
    "11": {
     "avg": -0.54,
     "hit": 50,
     "n": 10
    },
    "12": {
     "avg": -1.4,
     "hit": 10,
     "n": 10
    }
   },
   "AUDUSD": {
    "1": {
     "avg": 0.6,
     "hit": 55,
     "n": 11
    },
    "2": {
     "avg": -0.66,
     "hit": 45,
     "n": 11
    },
    "3": {
     "avg": -0.17,
     "hit": 45,
     "n": 11
    },
    "4": {
     "avg": 0.19,
     "hit": 36,
     "n": 11
    },
    "5": {
     "avg": -0.26,
     "hit": 55,
     "n": 11
    },
    "6": {
     "avg": 0.33,
     "hit": 64,
     "n": 11
    },
    "7": {
     "avg": 0.5,
     "hit": 64,
     "n": 11
    },
    "8": {
     "avg": -0.5,
     "hit": 30,
     "n": 10
    },
    "9": {
     "avg": -0.54,
     "hit": 50,
     "n": 10
    },
    "10": {
     "avg": -1.0,
     "hit": 20,
     "n": 10
    },
    "11": {
     "avg": 0.52,
     "hit": 50,
     "n": 10
    },
    "12": {
     "avg": 1.0,
     "hit": 70,
     "n": 10
    }
   },
   "NZDUSD": {
    "1": {
     "avg": 0.39,
     "hit": 64,
     "n": 11
    },
    "2": {
     "avg": -0.91,
     "hit": 27,
     "n": 11
    },
    "3": {
     "avg": -0.68,
     "hit": 55,
     "n": 11
    },
    "4": {
     "avg": -0.16,
     "hit": 45,
     "n": 11
    },
    "5": {
     "avg": 0.29,
     "hit": 64,
     "n": 11
    },
    "6": {
     "avg": 0.22,
     "hit": 55,
     "n": 11
    },
    "7": {
     "avg": 0.35,
     "hit": 73,
     "n": 11
    },
    "8": {
     "avg": -1.04,
     "hit": 50,
     "n": 10
    },
    "9": {
     "avg": -1.0,
     "hit": 50,
     "n": 10
    },
    "10": {
     "avg": -1.04,
     "hit": 40,
     "n": 10
    },
    "11": {
     "avg": 1.83,
     "hit": 60,
     "n": 10
    },
    "12": {
     "avg": 0.72,
     "hit": 70,
     "n": 10
    }
   },
   "US30": {
    "1": {
     "avg": 1.76,
     "hit": 70,
     "n": 10
    },
    "2": {
     "avg": -0.97,
     "hit": 50,
     "n": 10
    },
    "3": {
     "avg": -1.48,
     "hit": 50,
     "n": 10
    },
    "4": {
     "avg": 1.45,
     "hit": 70,
     "n": 10
    },
    "5": {
     "avg": 0.64,
     "hit": 80,
     "n": 10
    },
    "6": {
     "avg": 1.56,
     "hit": 70,
     "n": 10
    },
    "7": {
     "avg": 2.61,
     "hit": 90,
     "n": 10
    },
    "8": {
     "avg": 0.79,
     "hit": 60,
     "n": 10
    },
    "9": {
     "avg": -0.98,
     "hit": 50,
     "n": 10
    },
    "10": {
     "avg": 1.38,
     "hit": 50,
     "n": 10
    },
    "11": {
     "avg": 4.5,
     "hit": 90,
     "n": 10
    },
    "12": {
     "avg": 0.3,
     "hit": 70,
     "n": 10
    }
   },
   "GOLD": {
    "1": {
     "avg": 4.35,
     "hit": 73,
     "n": 11
    },
    "2": {
     "avg": 1.27,
     "hit": 55,
     "n": 11
    },
    "3": {
     "avg": 1.04,
     "hit": 55,
     "n": 11
    },
    "4": {
     "avg": 2.09,
     "hit": 73,
     "n": 11
    },
    "5": {
     "avg": -0.13,
     "hit": 36,
     "n": 11
    },
    "6": {
     "avg": -0.82,
     "hit": 36,
     "n": 11
    },
    "7": {
     "avg": 1.86,
     "hit": 73,
     "n": 11
    },
    "8": {
     "avg": 0.96,
     "hit": 40,
     "n": 10
    },
    "9": {
     "avg": -0.29,
     "hit": 30,
     "n": 10
    },
    "10": {
     "avg": 1.41,
     "hit": 60,
     "n": 10
    },
    "11": {
     "avg": -0.37,
     "hit": 60,
     "n": 10
    },
    "12": {
     "avg": 2.23,
     "hit": 80,
     "n": 10
    }
   }
  },
  "status": {
   "EURUSD": "ok",
   "GBPUSD": "ok",
   "USDCAD": "ok",
   "USDJPY": "ok",
   "USDCHF": "ok",
   "AUDUSD": "ok",
   "NZDUSD": "ok",
   "US30": "ok",
   "GOLD": "ok"
  }
 },
 "scorecard": {
  "weights": {
   "bnk": 3.0,
   "cb": 2.0,
   "ind": 1.0,
   "yld": 1.5,
   "cot": 1.0,
   "reg": 1.0,
   "sea": 0.5
  },
  "crit_order": [
   "bnk",
   "cb",
   "ind",
   "yld",
   "cot",
   "reg",
   "sea"
  ],
  "crit_label": {
   "cb": "CB",
   "bnk": "BNK",
   "ind": "IND",
   "yld": "YLD",
   "cot": "COT",
   "reg": "REG",
   "sea": "SEA"
  },
  "judgment_crit": [
   "bnk"
  ],
  "confirm_crit": [
   "bnk",
   "cb"
  ],
  "ccy_order": [
   "USD",
   "EUR",
   "GBP",
   "CAD",
   "JPY",
   "CHF",
   "AUD",
   "NZD"
  ],
  "month": 7,
  "regime_score": 2,
  "regime_src": "calculat",
  "regime_why": "VIX 14.90, percentila 8 pe 1 an · spread high yield 2.70%, percentila 9 pe 1 an · S&P +2.41% pe 20 de ședințe, +3.58% pe 5",
  "currencies": {
   "USD": {
    "scores": {
     "bnk": -1,
     "cb": 0,
     "ind": 0,
     "yld": 0,
     "cot": -2,
     "reg": -1,
     "sea": -1
    },
    "why": {
     "bnk": "Singura teză direcțională (ING, bullish DXY) a fost scrisă înainte de datele pieței muncii și premisa s-a rupt. Nicio casă nu mai apără un long de dolar.",
     "cb": "edge -25bp — aliniat cu pricing-ul (sub pragul de 25bp)",
     "ind": "etichetă manuală „mixt\"",
     "yld": "Δ2Y +0.020, adică +0.030 față de media grupului",
     "cot": "long aglomerat (percentila 100) — vulnerabil la dezamăgire",
     "reg": "regim +2 × beta -0.5 (VIX 14.90, percentila 8 pe 1 an · spread high yield 2.70%, percentila 9 pe 1 an · S&P +2.41% pe 20 de ședințe, +3.58% pe 5)",
     "sea": "media lunii -0.54%, hit 31% (7 perechi)"
    },
    "total": -3.2,
    "missing": [],
    "contra": "",
    "catalyst": "Minute FOMC 8.07",
    "rank": 7
   },
   "EUR": {
    "scores": {
     "bnk": 0,
     "cb": 0,
     "ind": 0,
     "yld": -1,
     "cot": 2,
     "reg": 0,
     "sea": 1
    },
    "why": {
     "bnk": "UniCredit întors în neutru: niciun bloc nu are motiv propriu să tragă cursul. Consens că rămâne piață de intervale.",
     "cb": "edge -17bp — aliniat cu pricing-ul (sub pragul de 25bp)",
     "ind": "etichetă manuală „mixt\"",
     "yld": "Δ2Y -0.095, adică -0.102 față de media grupului",
     "cot": "short aglomerat (percentila 8) — combustibil de squeeze",
     "reg": "regim +2 × beta +0.0 (VIX 14.90, percentila 8 pe 1 an · spread high yield 2.70%, percentila 9 pe 1 an · S&P +2.41% pe 20 de ședințe, +3.58% pe 5)",
     "sea": "media lunii +0.46%, hit 73% (1 perechi)"
    },
    "total": 0.5,
    "missing": [],
    "contra": "",
    "catalyst": "",
    "rank": 6
   },
   "GBP": {
    "scores": {
     "bnk": -1,
     "cb": -2,
     "ind": -1,
     "yld": -2,
     "cot": -1,
     "reg": -1,
     "sea": 1
    },
    "why": {
     "bnk": "Cea mai slabă acoperire din inbox: o singură casă (ING), bearish și explicit fragilă, fără ținte și fără orizont.",
     "cb": "edge -94bp — deja prețuit peste view-ul desk-ului",
     "ind": "etichetă manuală „negativ\"",
     "yld": "Δ2Y -0.113, adică -0.122 față de media grupului",
     "cot": "long aglomerat (percentila 85) — vulnerabil la dezamăgire",
     "reg": "regim +2 × beta -0.3 (VIX 14.90, percentila 8 pe 1 an · spread high yield 2.70%, percentila 9 pe 1 an · S&P +2.41% pe 20 de ședințe, +3.58% pe 5)",
     "sea": "media lunii +0.44%, hit 73% (1 perechi)"
    },
    "total": -6.2,
    "missing": [],
    "contra": "",
    "catalyst": "",
    "rank": 8
   },
   "CAD": {
    "scores": {
     "bnk": 2,
     "cb": -1,
     "ind": 1,
     "yld": 2,
     "cot": 2,
     "reg": 2,
     "sea": 0
    },
    "why": {
     "bnk": "Singurul bloc unde trei case spun același lucru din unghiuri diferite: CACIB nivelurile (1,418 → 1,35-1,40), Scotiabank creșterea, CIBC dobânda.",
     "cb": "edge -46bp — deja prețuit peste view-ul desk-ului",
     "ind": "etichetă manuală „pozitiv\"",
     "yld": "Δ2Y +0.110, adică +0.133 față de media grupului",
     "cot": "short aglomerat (percentila 4) — combustibil de squeeze",
     "reg": "regim +2 × beta +1.0 (VIX 14.90, percentila 8 pe 1 an · spread high yield 2.70%, percentila 9 pe 1 an · S&P +2.41% pe 20 de ședințe, +3.58% pe 5)",
     "sea": "media lunii +0.40%, hit 55% (1 perechi)"
    },
    "total": 6.0,
    "missing": [],
    "contra": "",
    "catalyst": "BoC + MPR 15.07",
    "rank": 1
   },
   "JPY": {
    "scores": {
     "bnk": 0,
     "cb": 0,
     "ind": 0,
     "yld": 2,
     "cot": 0,
     "reg": -2,
     "sea": 1
    },
    "why": {
     "bnk": "Bloc rupt în două: CACIB ținte 162-163 pe carry, MUFG avertizează că trade-ul e prea aglomerat. Argumente solide, concluzii opuse — direcția netă se anulează.",
     "cb": "edge -11bp — aliniat cu pricing-ul (sub pragul de 25bp)",
     "ind": "etichetă manuală „neutru\"",
     "yld": "Δ2Y +0.104, adică +0.126 față de media grupului",
     "cot": "poziționare neutră (percentila 50)",
     "reg": "regim +2 × beta -1.0 (VIX 14.90, percentila 8 pe 1 an · spread high yield 2.70%, percentila 9 pe 1 an · S&P +2.41% pe 20 de ședințe, +3.58% pe 5)",
     "sea": "media lunii +0.89%, hit 73% (1 perechi)"
    },
    "total": 0.8,
    "missing": [],
    "contra": "CACIB vs. MUFG — scor plafonat la 0",
    "catalyst": "",
    "rank": 4
   },
   "CHF": {
    "scores": {
     "bnk": 0,
     "cb": -1,
     "ind": 0,
     "yld": 0,
     "cot": 2,
     "reg": 1,
     "sea": 1
    },
    "why": {
     "bnk": "Nicio casă nu deschide teză. CACIB semnalează doar anomalia de corelație: francul se mișcă pozitiv cu apetitul de risc, atipic pentru un refugiu.",
     "cb": "edge -44bp — deja prețuit peste view-ul desk-ului [citire pe 10Y — 2Y indisponibil]",
     "ind": "etichetă manuală „neutru\"",
     "yld": "Δ10Y -0.038, adică -0.036 față de media grupului [10Y — 2Y indisponibil]",
     "cot": "short aglomerat (percentila 8) — combustibil de squeeze",
     "reg": "regim +2 × beta +0.5 (VIX 14.90, percentila 8 pe 1 an · spread high yield 2.70%, percentila 9 pe 1 an · S&P +2.41% pe 20 de ședințe, +3.58% pe 5)",
     "sea": "media lunii +0.76%, hit 73% (1 perechi)"
    },
    "total": 0.8,
    "missing": [],
    "contra": "",
    "catalyst": "",
    "rank": 5
   },
   "AUD": {
    "scores": {
     "bnk": 1,
     "cb": 1,
     "ind": 0,
     "yld": 0,
     "cot": 0,
     "reg": 2,
     "sea": 0
    },
    "why": {
     "bnk": "Fără raport dedicat; intră în tema CACIB a „monedelor bătute\" care urmează să fie reevaluate. Teză de coș, nu de pereche.",
     "cb": "edge +36bp — piața prețuiește mai puțin decât vede desk-ul",
     "ind": "etichetă manuală „mixt\"",
     "yld": "Δ2Y -0.017, adică -0.012 față de media grupului",
     "cot": "poziționare neutră (percentila 58)",
     "reg": "regim +2 × beta +1.0 (VIX 14.90, percentila 8 pe 1 an · spread high yield 2.70%, percentila 9 pe 1 an · S&P +2.41% pe 20 de ședințe, +3.58% pe 5)",
     "sea": "media lunii +0.50%, hit 64% (1 perechi)"
    },
    "total": 3.5,
    "missing": [],
    "contra": "",
    "catalyst": "",
    "rank": 2
   },
   "NZD": {
    "scores": {
     "bnk": 1,
     "cb": -2,
     "ind": 1,
     "yld": 0,
     "cot": 2,
     "reg": 2,
     "sea": 1
    },
    "why": {
     "bnk": "Sinteza vine din TMV, nu dintr-o casă comercială: drumul cu cea mai mică rezistență duce spre o monedă mai fermă, dar cu câștig explicit limitat — majorarea e prețuită.",
     "cb": "edge -90bp — deja prețuit peste view-ul desk-ului",
     "ind": "etichetă manuală „pozitiv\"",
     "yld": "Δ2Y -0.020, adică -0.016 față de media grupului",
     "cot": "short aglomerat (percentila 4) — combustibil de squeeze",
     "reg": "regim +2 × beta +1.0 (VIX 14.90, percentila 8 pe 1 an · spread high yield 2.70%, percentila 9 pe 1 an · S&P +2.41% pe 20 de ședințe, +3.58% pe 5)",
     "sea": "media lunii +0.35%, hit 73% (1 perechi)"
    },
    "total": 2.2,
    "missing": [],
    "contra": "",
    "catalyst": "Decizie RBNZ 8.07",
    "rank": 3
   }
  },
  "ranked": [
   "CAD",
   "AUD",
   "NZD",
   "JPY",
   "CHF",
   "EUR",
   "USD",
   "GBP"
  ],
  "pairs": [
   {
    "pair": "USDCAD",
    "gap": -9.2,
    "band": "STRONG",
    "dir": "SHORT",
    "verdict": "BOOK",
    "note": "diferență peste prag, criteriile 1-2 confirmă, catalizator în fereastră",
    "catalyst": "Minute FOMC 8.07 · BoC + MPR 15.07",
    "blind": [],
    "cross": false,
    "legs": {
     "base": "USD",
     "quote": "CAD",
     "base_total": -3.2,
     "quote_total": 6.0
    }
   },
   {
    "pair": "GBPNZD",
    "gap": -8.4,
    "band": "STRONG",
    "dir": "SHORT",
    "verdict": "BOOK",
    "note": "diferență peste prag, criteriile 1-2 confirmă, catalizator în fereastră",
    "catalyst": "Decizie RBNZ 8.07",
    "blind": [],
    "cross": true,
    "legs": {
     "base": "GBP",
     "quote": "NZD",
     "base_total": -6.2,
     "quote_total": 2.2
    }
   },
   {
    "pair": "GBPJPY",
    "gap": -7.0,
    "band": "CLEAR",
    "dir": "SHORT",
    "verdict": "WATCH",
    "note": "diferență peste prag, dar fără catalizator — rank fără eveniment sângerează timp",
    "catalyst": "",
    "blind": [],
    "cross": true,
    "legs": {
     "base": "GBP",
     "quote": "JPY",
     "base_total": -6.2,
     "quote_total": 0.8
    }
   },
   {
    "pair": "AUDUSD",
    "gap": 6.7,
    "band": "CLEAR",
    "dir": "LONG",
    "verdict": "BOOK",
    "note": "diferență peste prag, criteriile 1-2 confirmă, catalizator în fereastră",
    "catalyst": "Minute FOMC 8.07",
    "blind": [],
    "cross": false,
    "legs": {
     "base": "AUD",
     "quote": "USD",
     "base_total": 3.5,
     "quote_total": -3.2
    }
   },
   {
    "pair": "EURGBP",
    "gap": 6.7,
    "band": "CLEAR",
    "dir": "LONG",
    "verdict": "WATCH",
    "note": "diferență peste prag, dar fără catalizator — rank fără eveniment sângerează timp",
    "catalyst": "",
    "blind": [],
    "cross": true,
    "legs": {
     "base": "EUR",
     "quote": "GBP",
     "base_total": 0.5,
     "quote_total": -6.2
    }
   },
   {
    "pair": "NZDUSD",
    "gap": 5.4,
    "band": "THIN",
    "dir": "LONG",
    "verdict": "WATCH",
    "note": "diferență moderată — devine idee doar cu catalizator",
    "catalyst": "Decizie RBNZ 8.07 · Minute FOMC 8.07",
    "blind": [],
    "cross": false,
    "legs": {
     "base": "NZD",
     "quote": "USD",
     "base_total": 2.2,
     "quote_total": -3.2
    }
   },
   {
    "pair": "CADJPY",
    "gap": 5.2,
    "band": "THIN",
    "dir": "LONG",
    "verdict": "WATCH",
    "note": "diferență moderată — devine idee doar cu catalizator",
    "catalyst": "BoC + MPR 15.07",
    "blind": [],
    "cross": true,
    "legs": {
     "base": "CAD",
     "quote": "JPY",
     "base_total": 6.0,
     "quote_total": 0.8
    }
   },
   {
    "pair": "USDJPY",
    "gap": -4.0,
    "band": "NOISE",
    "dir": "SHORT",
    "verdict": "SUB PRAG",
    "note": "diferență în zona de zgomot — fără trade",
    "catalyst": "Minute FOMC 8.07",
    "blind": [],
    "cross": false,
    "legs": {
     "base": "USD",
     "quote": "JPY",
     "base_total": -3.2,
     "quote_total": 0.8
    }
   },
   {
    "pair": "USDCHF",
    "gap": -4.0,
    "band": "NOISE",
    "dir": "SHORT",
    "verdict": "SUB PRAG",
    "note": "diferență în zona de zgomot — fără trade",
    "catalyst": "Minute FOMC 8.07",
    "blind": [],
    "cross": false,
    "legs": {
     "base": "USD",
     "quote": "CHF",
     "base_total": -3.2,
     "quote_total": 0.8
    }
   },
   {
    "pair": "EURUSD",
    "gap": 3.7,
    "band": "NOISE",
    "dir": "LONG",
    "verdict": "SUB PRAG",
    "note": "diferență în zona de zgomot — fără trade",
    "catalyst": "Minute FOMC 8.07",
    "blind": [],
    "cross": false,
    "legs": {
     "base": "EUR",
     "quote": "USD",
     "base_total": 0.5,
     "quote_total": -3.2
    }
   },
   {
    "pair": "GBPUSD",
    "gap": -3.0,
    "band": "NOISE",
    "dir": "SHORT",
    "verdict": "SUB PRAG",
    "note": "diferență în zona de zgomot — fără trade",
    "catalyst": "Minute FOMC 8.07",
    "blind": [],
    "cross": false,
    "legs": {
     "base": "GBP",
     "quote": "USD",
     "base_total": -6.2,
     "quote_total": -3.2
    }
   },
   {
    "pair": "AUDNZD",
    "gap": 1.3,
    "band": "NOISE",
    "dir": "LONG",
    "verdict": "SUB PRAG",
    "note": "diferență în zona de zgomot — fără trade",
    "catalyst": "Decizie RBNZ 8.07",
    "blind": [],
    "cross": true,
    "legs": {
     "base": "AUD",
     "quote": "NZD",
     "base_total": 3.5,
     "quote_total": 2.2
    }
   },
   {
    "pair": "EURJPY",
    "gap": -0.3,
    "band": "NOISE",
    "dir": "SHORT",
    "verdict": "SUB PRAG",
    "note": "diferență în zona de zgomot — fără trade",
    "catalyst": "",
    "blind": [],
    "cross": true,
    "legs": {
     "base": "EUR",
     "quote": "JPY",
     "base_total": 0.5,
     "quote_total": 0.8
    }
   }
  ],
  "thresholds": {
   "book": 6.0,
   "watch": 5.0,
   "strong": 8.0
  }
 }
};
